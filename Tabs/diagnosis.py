...
import pandas as pd
import importlib
# Try to import FPDF dynamically; if not available we will disable PDF features.
FPDF = None
FPDF_AVAILABLE = False
try:
    _fpdf_mod = importlib.import_module("fpdf")
    FPDF = getattr(_fpdf_mod, "FPDF", None)
    if FPDF is not None:
        FPDF_AVAILABLE = True
except Exception:
    FPDF = None
...
# google.generativeai is imported dynamically below (optional dependency)
...
# Retrieve key and configure if available; don't raise — we will disable Gemini features when missing
GEMINI_API_KEY = get_gemini_api_key()
# Dynamic import for optional google.generativeai package
genai = None
    ...
    with tab2:
            ...
            def get_gemini_medication_recommendation(disease_type, patient_data):
                ...
                **Patient Data**:
                ...
                # If no API key is configured, return an informative message instead of calling Gemini.
                if not GEMINI_AVAILABLE:
                    return (
                        "Gemini API key is not configured or google.generativeai is not installed. "
                        "Medication recommendations are disabled. Add `GEMINI_API_KEY` to Streamlit secrets or set it in the environment and install the `google-generativeai` package to enable this feature."
                    )
                ...
                try:
                    ...
                    try:
                        ...
                        if disease_type != "No diabetes detected":
                            ...
                            # Call Gemini to generate medication recommendations (guarded inside function)
                            medication_info = get_gemini_medication_recommendation(disease_type, patient_data)

                            st.info("Medication Recommendation:")
